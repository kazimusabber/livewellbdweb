...
const clickDelete = (e, id) => {
    ...
    if(!isConfirm){
            ...
            http.delete('/message'+id).then((response) => {
             if (response.data) {
  ...
  
  {
      name: 'Action',
      cell:row =><div>
                  <Link to={`/admin/messageedit/${row.id}`} className='btn'>
    ...
    <div className="container">
      <div className="card">
        <div className="card-header"> Message List</div>
        <div className="card-body">
          <Datatablecomponent columns = {columns} url={`/message`}></Datatablecomponent>
        </div>
      </div>
    </div>
  );
