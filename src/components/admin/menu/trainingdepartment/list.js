...
const clickDelete = (e, id) => {
    ...
    if(!isConfirm){
            ...
            http.delete('/trainingdepartment/'+id).then((response) => {
             if (response.data) {
  ...
  
  {
      name: 'Action',
      cell:row =><div>
                  <Link to={`/admin/trainingdepartment/edit/${row.id}`} className='btn'>
  ...
  return (
    <div className="container">
      <div className="card">
        <div className="card-header"> Training Deparment List <Link to="/admin/trainingdepartment/create">
      <button type="button" className="btn btn-primary" style={{float: "right"}}>
        <i class="fas fa-plus" aria-hidden="true"> ADD</i>
      </button>
    </Link></div>
        <div className="card-body">
          <Datatablecomponent columns = {columns} url={`/trainingdepartment`}></Datatablecomponent>
        </div>
      </div>
    </div>
  );
